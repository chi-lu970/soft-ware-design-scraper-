...
| 屬性 | 類型 | 說明 |
|---|---|---|
| `keyword` | str | 使用者輸入的搜尋關鍵字（已去除前後空白） |
| `max_results` | int | 最多回傳筆數，預設 10 |
| `_raw_xml` | str \| None | 原始 XML 回應，`fetch()` 後才有值 |
| `RSS_URL_TEMPLATE` | str（類別變數） | Bing News RSS URL 模板 |
| `_NOISE_PATTERNS` | list[str]（類別變數） | 噪音樣板詞黑名單 |
...
### Requirement: 刊登時間擷取（pubDate）
系統 SHALL 從 RSS item 的 `<pubDate>` 標籤提取刊登時間，並格式化為 `YYYY/MM/DD HH:MM`。

> ⚠️ 此需求為實作新增，原規格未涵蓋，以實作為準。

#### Scenario: pubDate 存在
- **WHEN** RSS item 包含 `<pubDate>` 標籤（RFC 2822 格式，如 `Tue, 01 Apr 2026 08:00:00 GMT`）
- **THEN** 系統使用 `email.utils.parsedate_to_datetime` 解析並格式化為 `YYYY/MM/DD HH:MM`

#### Scenario: pubDate 不存在或解析失敗
- **WHEN** RSS item 缺少 `<pubDate>` 或時間格式不合法
- **THEN** 系統回傳空字串
...
- **THEN** 系統使用 `urllib.parse.urlparse` + `parse_qs` 解析 `url` 參數值並回傳原始文章網址
...
### Requirement: 結果依刊登時間排序
系統 SHALL 在 `run()` 方法中，將解析後的 DataFrame 依「刊登時間」欄位降序排列，時間為空的項目排在最後。

> ⚠️ 此需求為實作新增，原規格未涵蓋，以實作為準。

#### Scenario: 排序執行
- **WHEN** `run()` 完成 fetch 與 parse 後組成 DataFrame
- **THEN** 系統呼叫 `sort_values("刊登時間", ascending=False, na_position="last")` 並 reset index
